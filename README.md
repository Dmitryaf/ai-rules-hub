...
Проверка исходного состояния — отдельная задача без изменений проекта. Она классифицирует выбранные требования как `satisfied`, `gap`, `not applicable` или `unknown` и возвращает отчёт. Готовый запрос для AI находится в [`templates/PROJECT_AUDIT_PROMPT.md`](templates/PROJECT_AUDIT_PROMPT.md); отдельной команды аудита в CLI пока нет.
...
4. Откройте целевой проект в AI-агенте и отправьте [готовый запрос завершения подключения и первичного аудита](templates/PROJECT_AUDIT_PROMPT.md). Агент прочитает применённые правила, заполнит только локальную специфику и проверит остальной проект без изменений. Переносить общие правила вручную не нужно.
...
6. Просмотрите Git diff локального нормативного слоя и managed snapshot и зафиксируйте подключение обычным процессом проекта. Исправления найденных разрывов выполняйте только в отдельно согласованной задаче.
...
7. Отправьте AI-агенту [готовый запрос](templates/PROJECT_AUDIT_PROMPT.md). Он объединит маршруты с существующим `AGENTS.md`, заполнит локальную специфику и проведёт аудит без изменения остального проекта.
8. Запустите проектный `doctor` и `status`. Просмотрите ошибки целостности, оставшиеся placeholders и состояние snapshot; успешный `doctor` не заменяет отчёт аудита.
9. Просмотрите diff локального нормативного слоя и managed snapshot перед его commit. Найденные разрывы проекта оставьте для отдельной задачи на исправление.
