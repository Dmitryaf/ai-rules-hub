...
Baseline-review — отдельная read-only задача. Она классифицирует выбранные требования как `satisfied`, `gap`, `not applicable` или `unknown` и возвращает отчёт; специальный формат отчёта и команда аудита пока не вводятся.

### Remediation

Remediation начинается только после того, как владелец выбрал конкретные разрывы и разрешённую поверхность файлов. Обнаруженное несоответствие не расширяет scope подключения или baseline-review.
...
12. Просмотрите diff локального нормативного слоя и managed snapshot перед его commit. Найденные разрывы проекта оставьте для отдельного baseline-review или remediation.
