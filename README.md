...
→ выбранные профили
→ локальные правила и явно записанные исключения проекта
```
...
Синхронизация использует локальный manifest целевого проекта, SHA-256 lock-файл и режимы `Plan`/`Apply`. Она не перезаписывает локальные проектные правила и не удаляет orphan-файлы автоматически.
