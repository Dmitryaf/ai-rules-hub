...
Целостность уже установленного snapshot проверяется независимо: `doctor` вычисляет SHA-256 каждого файла по записи в lock. Для текстовых файлов окончания строк нормализуются в LF и hash считается по UTF-8 без BOM; бинарные файлы проверяются обычным SHA-256. Поэтому отсутствие нужной revision в локальном checkout не мешает обнаружить ручное изменение managed-копии.
