...
CLI создаст отсутствующие `AGENTS.md`, `.ai-rules/RULESET.md` и `.ai-rules/PROJECT_RULES.md`, но не перезапишет существующие файлы. Просмотрите `update`, выполните `update -Apply`, затем передайте AI-агенту [`PROJECT_AUDIT_PROMPT.md`](PROJECT_AUDIT_PROMPT.md). Агент объединит маршруты, заполнит локальную специфику и проведёт аудит без исправления проекта; общие правила вручную переносить не нужно. Полный пользовательский сценарий находится в корневом [`README.md`](../README.md).

При подключении существующего проекта шаблоны описывают фактическое состояние: ссылка ведёт только к реально существующему источнику, отсутствующий источник отмечается `не установлен`, а неизвестная проверка — `не установлена`. Не создавайте документацию, tooling или CI ради заполнения шаблона и не устраняйте найденные разрывы в рамках подключения; запишите их в `RULESET.md` для отдельного решения владельца.
