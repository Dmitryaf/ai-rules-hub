...
try {
    ...
    Assert-True -Condition ($coreRule -match '(?m)^## \d+\. Понятный язык\s*$' -and $coreRule -match 'Сначала сообщай главный вывод' -and $coreRule -match 'простыми словами' -and $coreRule -match 'редкий термин.*при первом использовании' -and $coreRule -match 'не теряй условия, ограничения, риски') -Message 'CORE must define the universal plain-language default without losing precision'
