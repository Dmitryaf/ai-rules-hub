...
try {
    ...
    Assert-True -Condition ($rootReadme -match 'templates/PROJECT_AUDIT_PROMPT\.md' -and $rootReadme -match 'Shared rules do not need to be copied manually' -and $templatesReadme -match 'PROJECT_AUDIT_PROMPT\.md.*в проект не копируется') -Message 'onboarding docs must route to the reusable audit prompt without copying general rules'
